...
global A E Ks G I sncon ro_t nsec dx ncon Node sNode_q ax ay az am dt Ty TT
flag = menu('Select',...
						'Dynamic',...
						'static',...
                        'static of dynamic');
...
% Deta1
%% deta 1
E= 29e9;    ro_t= 2500;     nu=0.2;     bbbb=0.5;   hhhh=0.3;   L=0.6;
...
elseif flag == 2
 ...
 %% function uw_element
function  [wabc,uabc,x_ele,xd_ele,xdd_ele]=uw_ele(ncon,X_sys,Xd_sys,Xdd_sys,sncon);
...
return
%% function Mass
function [M_ele]=calculate_M_element1(xa,xc,ro_t,A,I)
...
for i=1:3
    ...
    syms x
%     S1 =[ 1/2-1/2*xi, 1/2+1/2*xi];
    S2=[(1 - 3*x/dx + 2*x^2/dx^2),(4*x/dx - 4*x^2/dx^2),( -x/dx + 2*x^2/dx^2)];
    m11(i,j)=int(S2(i)*S2(j),0,dx);
    end
end
 %functions needed to be implemented by reduced integration
 
%  S3=[(x^2/dx^2)-(5*(x^3/dx^3)/4)-((x^4/dx^4)/2)+(3*(x^5/dx^5)/4);
%         (dx/4)*(x^2/dx^2 - x^3/dx^3 - x^4/dx^4 + x^5/dx^5);
%         1 - (2*x^2/dx^2) + (x^4/dx^4);
%         dx*(x/dx - (2*x^3/dx^3) + x^5/dx^5);
%         x^2/dx^2 + (5*(x^3/dx^3)/4)-((x^4/dx^4)/2)-(3*(x^5/dx^5)/4);
%         (dx/4)*(-x^2/dx^2 - x^3/dx^3 + x^4/dx^4 + x^5/dx^5)];
%     S3=[1- 23*x^2/dx^2 + 66*x^3/dx^3 - 68*x^4/dx^4 +24*x^5/dx^5,...
%             x - 6*x^2/dx + 13*x^3/dx^2 - 12*x^4/dx^3 + 4*x^5/dx^4,...
%             16*x^2/dx^2 - 32*x^3/dx^3 + 16*x^4/dx^4,...
%             -8*x^2/dx + 32*x^3/dx^2 - 40*x^4/dx^3 + 16*x^5/dx^4,...
%             7*x^2/dx^2 - 34*x^3/dx^3 + 52*x^4/dx^4 - 24*x^5/dx^5,...
%             -x^2/dx + 5*x^3/dx^2 - 8*x^4/dx^3 + 4*x^5/dx^4];
%     syms x%ok test
    S3=[1- (387/69)*x^2/dx^2 + (214/23)*x^3/dx^3 - (156/23)*x^4/dx^4 + (144/69)*x^5/dx^5,...
            x - (168/69)*x^2/dx + (51/23)*x^3/dx^2 - (26/23)*x^4/dx^3 + (24/69)*x^5/dx^4,...
            (416/69)*x^2/dx^2 - (192/23)*x^3/dx^3 - (32/23)*x^4/dx^4 + (256/69)*x^5/dx^5,...
            - (16/69)*x^2/dx + (64/23)*x^3/dx^2 - (112/23)*x^4/dx^3 + (160/69)*x^5/dx^4,...
            -(29/69)*x^2/dx^2 - (22/23)*x^3/dx^3 + (188/23)*x^4/dx^4 - (400/69)*x^5/dx^5,...
              (5/69)*x^2/dx + (3/23)*x^3/dx^2 - (34/23)*x^4/dx^3 + (88/69)*x^5/dx^4];
    syms x
for i=1:6
    for j=1:6
        MM(i,j)=int(S3(i)*S3(j),0,dx);
    end
end
%     m22=int(S3(1)*S3(1),0,dx);
%     m25=int(S3(1)*S3(2),0,dx);
%     m28=int(S3(1)*S3(3),0,dx);
%     m33=int(S3(4)*S3(4),0,dx);
%     m36=int(S3(4)*S3(5),0,dx);
%     m39=int(S3(4)*S3(6),0,dx);
%     m55=int(S3(2)*S3(2),0,dx);
%     m58=int(S3(2)*S3(3),0,dx);
%     m66=int(S3(5)*S3(5),0,dx);
%     m69=int(S3(5)*S3(6),0,dx);
%     m88=int(S3(3)*S3(3),0,dx);
%     m99=int(S3(6)*S3(6),0,dx);
%     m23=int(S3(1)*S3(4),0,dx);
%     m26=int(S3(1)*S3(5),0,dx);
%     m29=int(S3(1)*S3(6),0,dx);
%     m35=int(S3(4)*S3(2),0,dx);
%     m38=int(S3(4)*S3(3),0,dx);
%     m56=int(S3(2)*S3(5),0,dx);
%     m59=int(S3(2)*S3(6),0,dx);
%     m68=int(S3(5)*S3(3),0,dx);
%     m89=int(S3(3)*S3(6),0,dx);
%     m32=m23;    
%     m52=m25;    
%     m53=m35;    
%     m62=m26;    
...
%% function  Assembling_KT_R
function [F_sys,T_sys,R_sys]=T_R_sys_Dynamic(X_,xd,xdd,X,Xd,Xdd,sNode_q,Node,ncon,uabc,wabc,f0,q0,F_app,ax,ay,az,M_sys_con1,coef_damp,R_sys_con1)
...
T_sys=zeros(3*sNode_q,3*sNode_q);    F_sys=zeros(3*sNode_q,1);
R_sys=zeros(3*sNode_q,1);
    xa=Node(1,1);    xc=Node(2,1);
    dx=xc-xa;
% [m]=mass_mms(xa,xc,ro_t,A,I,dx);
%for speed analysis725lain
for el=1:sncon
    n1=ncon(el,1);    n2=ncon(el,2);
    xa=Node(n1,1);    xc=Node(n2,1);
    dx=xc-xa;
...
%     Xddii=XddI(rrr);
    ...
    Xddii=c0*(Xii-Xi) - c2*Xdi - c3*Xddi ;
%     Xddiii=c2*Xdi + c3*Xddi ;
    Xdii=Xdi + c6*Xddi + c7*Xddii ;
%     Xdii=(c1*(Xii-Xi) - c4*Xdi - c5*Xddi);
%     Xdiii=c4*Xdi + c6*Xddi;
...
%     [m]=mass_mms(xa,xc,ro_t,A,I,dx);%low speed
    M=M_ele*am;
%     M=m*am;
    C=coef_damp*K_ele;
    Keff = c0*M + c1*C + K_ele ;
%     Keff = K_ele ;
    Feff = F_ele + M*(c0*(Xi) + c2*Xdi + c3*Xddi) + C*(c1*(Xi) + c4*Xdi + c5*Xddi) ;
%     Feff = F_ele + M*(c2*Xdi + c3*Xddi) + C*(c4*Xdi + c5*Xddi) ;
%     Feff = F_ele ;%+ M*(c0*Xi + c2*Xdi + c3*Xddi) + C*(c1*Xi + c4*Xdi + c5*Xddi) ;
%     Feff = F_ele - M*Xddi - C*Xdi;
%     Feff = F_ele ;%+ M*(c0*Xi + c2*Xdi + c3*Xddi) + C*(c1*Xi + c4*Xdi + c5*Xddi) ;
%     R_ele=(Feff-Keff*X_ele');
%     R_ele=-(Feff-Keff*Xii);

    R_ele=(Keff*Xii + M*(Xddii) + C*(Xdii)) - Feff ;
%     R_ele=(Keff*Xii + M*xddi + C*(xdi)) - Feff ;
%     R_ele=(Keff*Xii) - Feff ;
...
%     P= (Keff*(Xi) + DYNAMIC + M*Xddi + C*Xdi);
%     P= (Keff*(Xi)) + DYNAMIC + (M*(Xddi) + C*(Xdi))*0;
%     P= K_ele*(Xi);
...
plot(inp_graph,load_increment_F0*(0:NI_F0-1))
 ...
 %% function Newmark 
function [X,Xd,Xdd] =Newmark(P_ele,M,C,K,F,dt,Xi,Xii,Xdi,Xddi,sNode_q,n)
persistent gamma_ beta_
if isempty(beta_)
    gamma_=1/2;              beta_=1/4;
end
...
function [] =dynamic(nt,v0,coef_damp,np_deg,max_iter,F0,x_ele,f0,q0)
    global  sncon ncon Node sNode_q ax ay az am dt Ty TT
    % ipmact load Ray Clough
    Load=zeros(1,nt);
    Load(1,1)=0;
    Load(1,2)=5;
...
%          F_app=10*sin(0.5*1.57*n*dt/Ty);%resonance
%          F_app=10*sin(1.57*n*dt/Ty);%normal
         F_app=10;
%          F_app=0;
%          F_app=Load(n+1);
%          F_app=F_app+F0;
...
%             [X_sys]=Boundary(E,ro_t,L,v0,sncon,dx,sNode_q,r,stress_cr);
            ...
            Xdd_sys=Xddmat(:,1);
            dXmat(1,1)=v0;
            [wabc,uabc,x_ele,xd_ele,xdd_ele]=uw_ele(ncon,X_sys,Xd_sys,Xdd_sys,sncon);
...
%                         X_=Xmat(:,n+1);
            ...
            [F_sys,T_sys,R_sys]=T_R_sys_Dynamic(X_,xd,xdd,Xi,Xdi,Xddi,sNode_q,Node,ncon,uabc,wabc,f0,q0,F_app,ax,ay,az,M_sys_con1,coef_damp,R_sys_con1);
%             [T_sys,R_sys]=T_R_sys_Dynamic(x_ele,xi_ele,xd_ele,xdd_ele,sNode_q,Node,ncon,uabc,wabc,f0,q0,F_app,ax,ay,az,M_sys_con1,coef_damp,R_sys_con1);
%             [T_sys,R_sys]=T_R_sys_Dynamic(x_ele,sNode_q,Node,ncon,uabc,wabc,f0,q0,F_app,ax,ay,az,M_sys_con1,coef_damp,X,Xd,Xdd,XI,XdI,XddI,R_sys_con1);
%             [T_sys,R_sys]=T_R_sys(x_ele,sNode_q,Node,ncon,uabc,wabc,f0,q0,F_app,ax,ay,az);%         Applying Boundary Conditions
            [T_sys_con1,R_sys_con1]=KT_R_sys_con2(T_sys,R_sys,np_deg,sNode_q);  % apply constraints Boundary conds)
...
%              k3=K_sys_con1;
            ...
            X_sysi=X_sys;
            P_ele=(M_sys_con1*xdd + C*xd + k3*X_) - F_sys;

            [X,Xd,Xdd] =Newmark(P_ele,M_sys_con1,C,k3,f,dt,Xi,X_sysi,Xdi,Xddi,sNode_q,n);
            Xmat(:,n+1)=X;
            Xdmat(:,n+1)=Xd;
            Xddmat(:,n+1)=Xdd;
%             XI=X;
...
%          toler=sqrt((X_sys_former-X_sys)*(X_sys_former-X_sys))/sqrt(X_sys_former*X_sys_former);
            if iter>max_iter
                break
                disp('k')
            end
