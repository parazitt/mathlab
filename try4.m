...
for i=1:3
    ...
    syms x
%     S1 =[ 1/2-1/2*xi, 1/2+1/2*xi];
    S2(1)=(2*x^2/dx^2)-(3*x/dx)+1;  S2(2)=(-4*x^2/dx^2)+(4*x/dx);    S2(3)=(2*x^2/dx^2)-x/dx;   %S1 =[ 1-x, x];
...
%% function  Assembling_KT_R
function [T_sys,R_sys]=T_R_sys_Dynamic(X_,X,Xd,Xdd,sNode_q,Node,ncon,uabc,wabc,f0,q0,F_app,ax,ay,az,M_sys_con1,coef_damp,R_sys_con1)
...
plot(inp_graph,load_increment_F0*(0:NI_F0-1))
 ...
 %% function Newmark 
function [X,Xd,Xdd] =Newmark(M,C,K,F,dt,Xi,Xii,Xdi,Xddi,sNode_q,n)
persistent gamma_ beta_
if isempty(beta_)
    gamma_=1/2;              beta_=1/4;
end
...
%         Assembling
            ...
            [T_sys,R_sys]=T_R_sys_Dynamic(X_,Xi,Xdi,Xddi,sNode_q,Node,ncon,uabc,wabc,f0,q0,F_app,ax,ay,az,M_sys_con1,coef_damp,R_sys_con1);
...
%              k3=K_sys_con1;
            ...
            X_sysi=X_sys;
            [X,Xd,Xdd] =Newmark(M_sys_con1,C,k3,f,dt,Xi,X_sysi,Xdi,Xddi,sNode_q,n);
            Xmat(:,n+1)=X;
            Xdmat(:,n+1)=Xd;
            Xddmat(:,n+1)=Xdd;
%             XI=X;
