...
switch st_analysis_type
    case 'linear'
        LTBT(sNode_q,Node,ncon,uabc,wabc,Fsys)
%     Xmat(:,n+1)=X_sys;
    case 'nonlinear'
%         wabc=zeros(sncon,3);      uabc=zeros(sncon,3);

            NTBT(sNode_q,Node,ncon,uabc,wabc,Fsys)   %nonlinear timoshenko beam theory
...
function LTBT(sNode_q,Node,ncon,uabc,wabc,Fsys)
...
function NTBT(sNode_q,Node,ncon,uabc,wabc,Fsys)
global Xmat Xdmat Xddmat toler iter max_iter np_deg coef_damp n ...
    M_sys_con1 dt
    while toler>0.001 && iter<max_iter
        iter=iter+1;
